...
cartogram_shp <- st_read("map/distrito.shp")


#create a state map
#state_cartogram_shp <- unionSpatialPolygons(cartogram_shp, cartogram_shp@data$ENTIDAD)
state_cartogram_shp <- st_union(st_as_sf(cartogram_shp), by = "ENTIDAD")
...
# This assumes 'lon' and 'lat' are the names of the columns with the longitude and latitude
districts_sf <- st_as_sf(districts, coords = c("lon", "lat"), crs = 4326)

# Assuming 'cartogram_shp' is your spatial data containing geometries
# and it has a column 'DISTRICT_ID' that corresponds to the 'id' in your 'districts' data frame
districts_sf <- cartogram_shp %>%
  left_join(districts, by = c("CLAVEGEO" = "id")) %>%
  st_as_sf()
